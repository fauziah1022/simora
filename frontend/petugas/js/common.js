...
function checkAuth() {
  ...
  const cabang = localStorage.getItem('simora_cabang');
  if (isLoggedIn !== 'true' || role !== 'petugas' || !cabang) {
    window.location.href = 'index.html';
  }
}

function getPetugasCabang() {
  return localStorage.getItem('simora_cabang') || '';
}
...
function doLogout() {
  ...
  localStorage.removeItem('simora_cabang');
  window.location.href = 'index.html';
}
...
function renderTopbar(title, subtitle) {
  const adminName = localStorage.getItem('simora_admin_name') || 'Petugas';
  const cabang = getPetugasCabang();
  ...
  return `
    <header class="topbar">
      ...
      <div class="topbar-right">
        <span class="badge badge-info" style="padding:8px 14px;"><i class="fa-solid fa-location-dot"></i> Cabang ${cabang}</span>
